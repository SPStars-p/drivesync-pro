...
// --- 11 FITUR TAMBAHAN DRIVE-SYNC ULTIMATE ---

// 1. Fitur Auto-Resume Upload (Melanjutkan upload yang terputus)
async function resumeInterruptedUploads() {
    const files = await getAllFilesFromDB();
    const stuckFiles = files.filter(f => f.status === 'uploading');
    for (const f of stuckFiles) {
        f.status = 'pending';
        await saveFileToDB(f);
    }
    if (stuckFiles.length > 0) {
        showToast(`${stuckFiles.length} file macet dikembalikan ke antrean (Auto-Resume).`, "info");
        processQueue();
    }
}

// 2. Fitur Drag-and-Drop Multiple Folders Rekursif
async function handleDirectoryEntry(item, path = '') {
    if (item.isFile) {
        return new Promise((resolve) => {
        ...
        for (let entry of entries) {
            let subFiles = await handleDirectoryEntry(entry, path + item.name + '/');
...
// 3. Fitur File Sharing & Permission (Generate Link Hak Akses Publik)
async function generatePublicShareLink(fileId) {
    const allFiles = await getAllFilesFromDB();
    const file = allFiles.find(f => f.id === fileId);
    if (!file || !file.driveFileId) {
        showToast("File belum tersinkronisasi ke cloud.", "warning");
        return;
    }
    const acc = accounts.find(a => a.id === file.targetAccountId);
    if (!acc || !isTokenValid(acc)) {
        showToast("Token akun tidak valid untuk mengatur izin.", "error");
        return;
    }
    try {
        await fetch(`https://www.googleapis.com/drive/v3/files/${file.driveFileId}/permissions`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${acc.token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ role: 'reader', type: 'anyone' })
        });
        showToast("Izin akses publik berhasil diaktifkan untuk file ini!", "success");
        logAuditAction("PERMISSION", `File ${file.name} diubah menjadi akses publik.`);
    } catch (e) {
        showToast("Gagal mengubah izin file.", "error");
    }
}

// 4. Fitur Real-time Sync Speedometer & Estimasi Waktu (ETA)
let uploadStartTime = 0;
let uploadedBytesTotal = 0;

function updateSpeedometer(loadedBytes, totalBytes) {
    const now = Date.now();
    const duration = (now - uploadStartTime) / 1000;
    if (duration <= 0) return;
    const speedBps = loadedBytes / duration;
    const remainingBytes = totalBytes - loadedBytes;
    const etaSeconds = speedBps > 0 ? Math.round(remainingBytes / speedBps) : 0;

    const speedEl = document.getElementById('sync-speed-indicator');
    if (speedEl) {
        speedEl.innerText = `${formatBytes(speedBps)}/s (ETA: ${etaSeconds}s)`;
    }
}

// 5. Fitur Search Filter Lanjutan berdasarkan Ekstensi File
function filterFilesByExtension(ext) {
    const searchInput = document.getElementById('explorer-search');
    if (searchInput) {
        searchInput.value = ext;
        renderExplorerUI();
    }
}

// 6. Fitur Cloud Quota Warning & Notifikasi Penuh (>90%)
async function checkStorageQuotaWarnings() {
    for (const acc of accounts) {
        if (acc.quota && acc.quota.limit > 0) {
            const usageRatio = acc.quota.usage / acc.quota.limit;
            if (usageRatio > 0.9) {
                showToast(`Peringatan: Penyimpanan akun ${acc.name} hampir penuh (>90%)!`, "error");
                sendTelegramNotification(`⚠️ *Peringatan Kapasitas Google Drive*\nAkun ${acc.name} hampir penuh!`);
            }
        }
    }
}

// 7. Fitur Trash Bin / Pemulihan File Lokal Terhapus (Soft Delete)
async function moveToTrashDB(fileId) {
    const allFiles = await getAllFilesFromDB();
    const file = allFiles.find(f => f.id === fileId);
    if (!file) return;
    file.isDeleted = true;
    ...
    await loadFilesFromDB();
    showToast("File dipindahkan ke Tempat Sampah (Trash).", "info");
    logAuditAction("TRASH", `File ${file.name} dimasukkan ke Trash Bin.`);
}

// 8. Fitur Bulk Actions (Hapus atau Pilih Banyak File Sekaligus)
async function bulkDeleteFiles(fileIdList) {
    if (!confirm(`Hapus ${fileIdList.length} file sekaligus?`)) return;
    for (const id of fileIdList) {
        await deleteFileFromDB(id);
    }
    await loadFilesFromDB();
    showToast(`${fileIdList.length} file berhasil dihapus massal.`, "success");
    logAuditAction("BULK", `${fileIdList.length} file dihapus secara massal.`);
}

// 9. Fitur Background Sync via Web Workers/Interval Fallback
setInterval(() => {
...
// 10. Fitur Dark/Light Theme Switcher Tambahan
function toggleAppTheme() {
    document.body.classList.toggle('light-theme');
    const isLight = document.body.classList.contains('light-theme');
    localStorage.setItem('drivesync_theme', isLight ? 'light' : 'dark');
}

// 11. Fitur Activity Log Export ke format CSV
async function exportAuditLogsCSV() {
    ...
    let csvContent = "data:text/csv;charset=utf-8,ID,Timestamp,Action,Detail\n";
    logs.forEach(l => {
        csvContent += `"${l.id}","${l.timestamp}","${l.action}","${escapeHtml(l.detail)}"\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "drivesync_audit_logs.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast("Log audit berhasil diekspor ke CSV!", "success");
}

document.addEventListener('DOMContentLoaded', async () => {
    // Kode inisialisasi aplikasi Anda yang sudah ada sebelumnya
    // (misalnya: loadFilesFromDB(), initAuth(), dll)

    // --> TARUH KEDUA BARIS INI DI SINI <--
    resumeInterruptedUploads();
    checkStorageQuotaWarnings();
});

// --- FITUR AUTO-CONNECT AKUN GOOGLE ---

// Menyimpan daftar akun yang sudah terhubung ke localStorage
function saveAccountsSession() {
    if (typeof accounts !== 'undefined') {
        // Simpan data penting akun (nama, email, status, dan token jika ada)
        const sessionData = accounts.map(acc => ({
            id: acc.id,
            name: acc.name,
            email: acc.email,
            isAuthenticated: acc.isAuthenticated || false,
            token: acc.token || null,
            tokenExpiry: acc.tokenExpiry || null
        }));
        localStorage.setItem('drivesync_saved_accounts', JSON.stringify(sessionData));
    }
}

// Memuat kembali sesi akun secara otomatis saat halaman dibuka
function restoreAccountsSession() {
    const saved = localStorage.getItem('drivesync_saved_accounts');
    if (saved && typeof accounts !== 'undefined') {
        try {
            const parsedAccounts = JSON.parse(saved);
            parsedAccounts.forEach(savedAcc => {
                const target = accounts.find(a => a.id === savedAcc.id);
                if (target) {
                    target.isAuthenticated = savedAcc.isAuthenticated;
                    target.token = savedAcc.token;
                    target.tokenExpiry = savedAcc.tokenExpiry;
                }
            });
            // Perbarui tampilan antarmuka akun di sidebar
            if (typeof renderAccountsUI === 'function') {
                renderAccountsUI();
            }
        } catch (e) {
            console.error("Gagal memulihkan sesi akun:", e);
        }
    }
}

document.addEventListener('DOMContentLoaded', async () => {
    // Panggil pemulihan sesi otomatis di awal
    restoreAccountsSession();

    // Fungsi bawaan lainnya...
    if (typeof resumeInterruptedUploads === 'function') resumeInterruptedUploads();
    if (typeof checkStorageQuotaWarnings === 'function') checkStorageQuotaWarnings();
});

// --- FITUR TAMBAHAN: WHATSAPP NOTIFICATION ---
async function sendWhatsAppNotification(phoneNumber, message) {
    // Anda bisa menggunakan API WhatsApp gateway gratis/berbayar (seperti Fonnte, Wablas, atau API pribadi)
    // Contoh menggunakan Fonnte API:
    const apiKey = localStorage.getItem('drivesync_wa_token'); // Token API Gateway WhatsApp Anda
    if (!apiKey) {
        showToast("Token API WhatsApp belum diatur di pengaturan.", "warning");
        return;
    }
    ...
    try {
        const response = await fetch('https://api.fonnte.com/send', {
            ...
            body: JSON.stringify({
                target: phoneNumber, // Nomor tujuan (contoh: 081234567890)
                message: message
            })
        });
        ...
        if (result.status) {
            ...
            showToast("Gagal mengirim WhatsApp: " + (result.reason || "Kesalahan sistem"), "error");
        }
    } catch (e) {
        console.error("Error WhatsApp API:", e);
        showToast("Gagal terhubung ke server WhatsApp.", "error");
    }
}

function openWhatsAppDirectChat(phoneNumber, message) {
